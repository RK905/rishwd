...
function filetripDeleteDir($dirPath) 
{
  ...
  function itf_install_filetrip_database()
  {
    ...
    /* */
    $all_columns = $wpdb->get_row("SELECT * FROM ". $wpdb->prefix . Filetrip_Constants::RECORD_TABLE_NAME, ARRAY_A);

    //Add column if not present.
    if(!isset($all_columns['is_deleted'])) {
        $result = $wpdb->query("ALTER TABLE ". $wpdb->prefix . Filetrip_Constants::RECORD_TABLE_NAME." ADD is_deleted tinyint(1) NOT NULL DEFAULT '0' ");
    }

    // delete all old fields that are obsolete
    foreach($all_columns as $key => $value)
    {
        switch($key)
        {
            ...
            case 'ftp_sent':
            case 'short_url':
                $wpdb->query("ALTER TABLE " . $wpdb->prefix . Filetrip_Constants::RECORD_TABLE_NAME . " DROP " . $key);
                break;
            default:
                break;
        }

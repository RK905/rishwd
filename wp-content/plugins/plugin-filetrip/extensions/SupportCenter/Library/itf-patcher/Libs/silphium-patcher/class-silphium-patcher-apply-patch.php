...
class Silphium_Patcher_Apply_Patch {
	...
	public function get_setting( $key ) {
		...
		$setting = get_option( 'silphium_patch_contents_' . $key, false );
		...
		if ( is_array( $this->setting ) ) {
			...
			foreach ( $this->setting as $target => $args ) {
				...
				foreach ( $args as $destination => $source ) {
					new Silphium_Patcher_Filesystem( $target, $source, $destination );
				}
			}
	...
	public function remove_setting( $key ) {
		delete_option( 'silphium_patch_contents_' . $key );
	}
	...
	public function update_applied_patches( $key ) {
		...
		if ( ! in_array( $key, $applied_patches ) ) {
			...
			update_site_option( 'silphium_applied_patches', $applied_patches );
		}
	}
}

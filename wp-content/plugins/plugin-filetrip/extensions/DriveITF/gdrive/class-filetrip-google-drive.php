...
require_once __DIR__ . '/_setting_page.php';
  ...
  class Filetrip_Google_Drive
  {
    ...
    static public $google_drive_client_secret_slug = 'client_secret';
    static public $google_drive_client_id_slug = 'client_id';
    static public $google_redirect_url_slug = 'redirect_url';
    ...
    private function authenticate($code, $setting)
    {
        ...
        // Finalize and store important token handshake information 
        $this->save_new_token($this->client->getAccessToken(), $setting);
    }
    ...
    private function save_new_token($accessToken, $setting)
    {
      ...
      $setting[Filetrip_Google_Drive::$google_drive_refresh_token_slug] = isset($accessToken['refresh_token'])?$accessToken['refresh_token']:'';
    ...
    public function get_folder_list_html()
    {
      ...
      ?>
        <div class="arfaly-drive-folder-loading spinner"></div>
            ...
            <h3>Path: <b><?php echo $file_info['name']; ?></b></h3>
            <?php if ( empty( $files ) ) : ?>
            <?php echo _e( 'There is no folders', 'filetrip-plugin' ); ?>
            <ul class="arfaly-file-listing">
                <li class="arfaly-drive-get-back">
                  <a href="javaScript: void(0);" onclick="javascript:arfaly_drive_get_folder_list('')">(Root)</a>
                </li>
            </ul>
            <?php else : ?>
            <ul class="arfaly-file-listing">
              ...
              foreach( $files as $file ) : ?>
                    <?php if ( $file['mimeType'] == 'application/vnd.google-apps.folder' ) : ?>
                        <li class="arfaly-folder-drive">
                          <a href="javaScript: void(0);" onclick="javascript:arfaly_update_drive_folder('<?php echo '('.$file['name'].') */* '.$file['id']; ?>', '<?php echo $switch; ?>')" class="button">Select</a>
                            <?php echo '<img src="'.$file['iconLink'].'">'; ?>
                          <a href="javaScript: void(0);" onclick="javascript:arfaly_drive_get_folder_list('<?php echo $file['id']; ?>')"><?php echo $file['name'] ;?></a>
                        </li>
                    <?php endif; ?>
                <?php endforeach; ?>
            </ul>
    ...
    { 
        ...
        try {
          ...
          if ($pageToken) {
            ...
            if($query == '')
            {
              ...
              $parameters['q'] = "mimeType='application/vnd.google-apps.folder' and '".$query."' in parents and trashed=false";
            }
          }
          $setting = Filetrip_Google_Drive::get_google_drive_settings();
	  ...
	  public static function generate_shared_link($fileId){

        if($fileId){
	        $setting = Filetrip_Google_Drive::get_google_drive_settings();
	        $client = Filetrip_Google_Drive::build_client_object($setting);
	        $driveService = new Google_Service_Drive($client);
	        $gdrive_shared_url = "https://drive.google.com/open?";
	        $param = ['id' => $fileId ];
	        try {
            ...
            $driveService->permissions->create($fileId, $newPermission);
		        ...
		        return $gdrive_shared_url.http_build_query($param);
	        } catch (Exception $e){
		        if ($e->getCode() === 404) return false;
		        if ($e->getCode() !== 200) do_action('itech_error_caught', $e->getMessage());
		        return false;
	        }
        }
		  return false;
    ...
    public static function resumable_file_upload($filesize, $title, $description, $mimeType, $filepath, $folder_id = '', $sse_enabled = true, $att_id=null)
    {
      set_time_limit(0);
      session_write_close();
...
//        new Google_Service_Drive_Permission();
        ...
        /* Uploading process has finished */
	      if($att_id != null){
	        do_action('itf/filetrip/gdrive/file/uploded', $result, $att_id);
        }

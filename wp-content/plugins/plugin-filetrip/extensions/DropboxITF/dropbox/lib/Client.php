...
class Client
{
    ...
    {
        $response = $this->doPost($this->apiHost, "oauth2/token/revoke");
    ...
    {
        $response = $this->doPost($this->apiHost, "/2/users/get_current_account");
        if ($response->statusCode !== 200) throw RequestUtil::unexpectedStatus($response);
     ...
     * @throws Exception
     */
    function chunkedUploadContinue($uploadId, $byteOffset, $data, $close = false)
    {
        Checker::argStringNonEmpty("uploadId", $uploadId);
        Checker::argNat("byteOffset", $byteOffset);
        Checker::argString("data", $data);

        $response = $this->_chunkedUploadAppendv2(
            array(
                'cursor'=>array(
                  'session_id' => $uploadId,
                  'offset' => $byteOffset
     ...
     * @throws Exception
     */
    function chunkedUploadFinish($uploadId, $path, $offset, $writeMode)
    {
        Checker::argStringNonEmpty("uploadId", $uploadId);
        Path::checkArgNonRoot("path", $path);
        WriteMode::checkArg("writeMode", $writeMode);
        ...
        $params = array(
              ...
              'commit'=>array(
                  'path' => $path,
                  'mode' => $writeMode->getExtraParams(),
                  'autorename' => true,
                  'mute' => false
              )
            );

        $response = $this->doUploadPost(
            $this->contentHost,
        ...
        // Marked 'protected' so I can override it in testing.
    {
        ...
        return $this->doUploadPost($this->contentHost,
     ...
     * @return HttpResponse
     */
    protected function _chunkedUploadAppendv2($params, $data)
        // Marked 'protected' so I can override it in testing.
    {
     ...
     * @return HttpResponse
     */
    protected function _chunkedUploadAppend($params, $data)
        // Marked 'protected' so I can override it in testing.
    {
        $url = $this->buildUrlForGetOrPut(
            $this->contentHost, "upload_session/append_v2", $params);
    ...
    function getMetadataWithChildren($path)
    {
        ...
        return $this->_getMetadata( array(
    ...
    /**
     * Returns the list of folders and files for a specific path
     *
     * @param string $path
     *    The Dropbox path to a file or folder (UTF-8).
     *
    ...
    function getFolderList($path)
    {
        ...
        return $this->_getFolderList( array(
              "path" => $path,
              "include_media_info" => false, 
              "include_deleted" => false, 
              "include_has_explicit_shared_members" => false,
     ...
     * @return array
     */
    private function _getFolderList($params)
    {
        ...
        $folderList = RequestUtil::parseResponseJson($response->body);
    ...
    function get_shareable_link($param){
	    $response = $this->doPost(
		    $this->apiHost,
     ...
     * @throws Exception
     */
    function createShareableLink($param)
    {
        //Path::checkArg("path", $param);
	    ...
	    if($response->statusCode === 409){
		 $response = $this->get_shareable_link($param);
	    }
	    ...
	    $body = RequestUtil::parseResponseJson($response->body);
	    if(!key_exists('url',$body)){
	    	$body = $body['links'][0];
	    }
	    return self::getField($body, "url");
    }
    ...
    /**
     * Perform an OAuth-2-authorized CONTENT POST request to the Dropbox API.  Will automatically
     * fill in "User-Agent" as well.
     *
     * @param string $host
     *    Either the "API" or "API content" hostname from {@link getHost()}.
     * @param string $path
     ...
     * @throws Exception
     */
    function doUploadPost($host, $path, $data, $params = null)
    {
        Checker::argString("host", $host);
        Checker::argString("path", $path);
        return RequestUtil::doUploadPost($this->clientIdentifier, $this->accessToken,
                                   $host, $path, $data, $params);
    }

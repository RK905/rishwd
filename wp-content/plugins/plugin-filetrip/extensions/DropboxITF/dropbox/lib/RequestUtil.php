...
final class RequestUtil
{
    ...
    static function buildJsonPostBody($params)
    {
     ...
     * @throws Exception
     */
    static function doUploadPost($clientIdentifier, $accessToken, $host, $path, $data, $params = null)
    {
        // DEBUG HEADER =================
        // error_log(self::buildJsonPostBody($params));

        $dropboxHeaderArgs = empty($params)?'{}':self::buildJsonPostBody($params);
        ...
        // $curl->set(CURLOPT_VERBOSE, TRUE);
        // $curl->set(CURLOPT_STDERR, $verbose);
        
        $curl->addHeader("Content-Type: application/octet-stream");
        $curl->addHeader("Dropbox-API-Arg: " . $dropboxHeaderArgs);
        ...
        // DEBUG HEADER =================
        // rewind($verbose);
        // error_log(stream_get_contents($verbose));
     ...
     * @throws Exception
     */
    static function doPost($clientIdentifier, $accessToken, $host, $path, $params = null)
    {
        //error_log(self::buildJsonPostBody($params));
        ...
        $curl = self::mkCurlWithOAuth($clientIdentifier, $url, $accessToken);
        $curl->set(CURLOPT_POST, true);
        $curl->set(CURLOPT_POSTFIELDS, self::buildJsonPostBody($params));

...
class Arfaly_Report_Statistic {
  ...
  */
  ...
  function get_report_messages_by_date($ranged, $from=0, $to=0)
  {
    ...
    if($ranged){
      $results = $wpdb->get_results( 'SELECT * FROM '.$this->db_table.' WHERE is_deleted = 0 AND time BETWEEN "'.$from.' 00:00:00" AND "'.$to. ' 23:59:59"' , ARRAY_A );
    }else{
      $results = $wpdb->get_results( 'SELECT * FROM '.$this->db_table . ' WHERE is_deleted = 0' , ARRAY_A );
    }

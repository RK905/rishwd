...
class GF_Field_Filetrip extends GF_Field {
	...
	public function get_value_save_entry( $value, $form, $input_name, $lead_id, $lead ) {
		...
		$uploads        = array();
		$filetrip_field = array_filter(
			$form['fields'],
			function ( $e ) {
				return $e->type == 'filetrip-uploader';
			}
		);

		if ( is_array( $filetrip_field ) ) {
			$this_field_key     = key( $filetrip_field );
			$form_object        = $form['fields'][ $this_field_key ];
			$filetrip_id        = $form_object->filetrip_shortcode;
		}

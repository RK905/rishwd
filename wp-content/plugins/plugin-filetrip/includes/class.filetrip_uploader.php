...
class Filetrip_Uploader
{
    ...
    private $filetrip_wordpress_link_met = 'filetrip_wordpress_link';
    ...
    public function arfaly_init()
    {
        ...
        if (is_array($files) && count($files) > 0) {
            foreach ($files as $file) {
                if (strpos($file, ('arfaly.part.'.date('d'))) === false && strpos($file, 'index.php')  === false) {
                    unlink($file);
                }
            }
        }
        ...
        add_filter( 'itf/filetrip/filter/channels/get/shared/link', array($this, 'get_shared_link_if_avaliable'), 40, 2);
    ...
    {
        // First print out the error to Wordpress error log
        error_log($exp);
    ...
    static function building_arfaly_container($id, $required = false)
    {
        ...
        $closify_options = Filetrip_Uploader::building_arfaly_metadata_array($id, $required);
    ...
    static function building_arfaly_metadata_array($id, $required = false)
    {
        ...
        if(isset($meta[Filetrip_Constants::METABOX_PREFIX.'multipart_uploader'])){
            $closify_info['chunkUpload'] = ($meta[Filetrip_Constants::METABOX_PREFIX.'multipart_uploader'][0]=='on')?'false':'true';
        }
    ...
    function itech_submit_arfaly()
    {
        ...
        if ($_FILES["SelectedFile"]["error"] > 0) {
            ...
            $att_attributes = wp_get_attachment_url($att_id); // returns an array
            if ($att_attributes) {
                $targetFileURLPath = $att_attributes;
            } else {
            ...
            if (isset($_POST['fileIndx']) && intval($_POST['fileIndx'])==0) {
    ...
    function handle_chunk_upload($maxFileSize, $post_id, $att_title = '', $att_desc = '')
    {
        ...
        if (!$chunks || $chunk == $chunks) {
            ...
            // If Title is empty, insert filename as a title
            $att_title = $fileNameInfo['filename'].'.'.$fileNameInfo['extension'];
    ...
    function approve_media()
    {
        ...
        if (isset($_GET['id'])) {
            ...
            $query_s = $query_s.'source='.Filetrip_Constants::TransferType('forward').'&';
    ...
    /* Get shared link for an attachment if avaliable
        This function has the lowest priority, so it can act as a failover channel in case
        nothing was selected
    */
	function get_shared_link_if_avaliable($shared_url_links, $att_id)
	{
		$att_info         = get_post( $att_id );
		if(empty($att_info)) return $shared_url_links;

        $shared_url_links[] = wp_get_attachment_url($att_id);
    ...
    function add_cdn_option_to_dropmenu($link_upload_cdn_options)
	{
		...
		foreach ( $selectedChannels as $select ) {
			if ( $select == $this::WORDPRESS_SLUG ) {
                $link_upload_cdn_options[$this->filetrip_wordpress_link_met] =  arfaly_get_icon('wordpress', ' Wordpress');
			}
		}

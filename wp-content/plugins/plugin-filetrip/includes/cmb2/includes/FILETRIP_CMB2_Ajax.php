...
class FILETRIP_CMB2_Ajax {
	...
	public function oembed_handler() {
		...
		// Verify our nonce
		if ( ! ( isset( $_REQUEST['filetrip_cmb_ajax_nonce'], $_REQUEST['oembed_url'] ) && wp_verify_nonce( $_REQUEST['filetrip_cmb_ajax_nonce'], 'ajax_nonce' ) ) ) {
			die();
		}
	...
	public function hijack_oembed_cache_get( $check, $object_id, $meta_key ) {
		...
		return ( 'options-page' === $this->object_type )
			? filetrip_cmb_options( $this->object_id )->get( $this->embed_args['cache_key'] )
	...
	public function oembed_cache_set( $meta_key, $meta_value ) {
		...
		return ( 'options-page' !== $this->object_type )
			...
			: filetrip_cmb_options( $this->object_id )->update( $this->embed_args['cache_key'], $meta_value, true );
	}

...
/**
 * FILETRIP_CMB field validation
 * @since  0.0.4
 * @method string _id()
 */
class FILETRIP_CMB2_Sanitize {

	/**
	 * A FILETRIP_CMB field object
	 ...
	 * @since 1.1.0
	 * @param object $field A FILETRIP_CMB field object
	 * @param mixed  $value Field value
	 */
	public function __construct( FILETRIP_CMB2_Field $field, $value ) {
		$this->field = $field;
		$this->value = $value;
	}
	...
	public function default_sanitization( $value ) {
		 ...
		 * @param object     $sanitizer  This `FILETRIP_CMB2_Sanitize` object
		 */
		$override_value = apply_filters( 'filetrip_cmb_sanitize_'. $this->field->type(), null, $value, $this->field->object_id, $this->field->args(), $this );
		/**
		 * DEPRECATED. See documentation above.
		 */
		$override_value = apply_filters( 'filetrip_cmb_validate_'. $this->field->type(), $override_value, $value, $this->field->object_id, $this->field->args(), $this );
	...
	public function text_datetime_timestamp_timezone( $value, $repeat = false ) {
		...
		if ( empty( $tzstring ) ) {
			$tzstring = filetrip_cmb_utils()->timezone_string();
		}

		$offset = filetrip_cmb_utils()->timezone_offset( $tzstring, true );
	...
	public function _save_file_id( $value ) {
		...
		if ( $value && ! $id_val ) {
			$id_val = filetrip_cmb_utils()->image_id_from_url( $value );
		}

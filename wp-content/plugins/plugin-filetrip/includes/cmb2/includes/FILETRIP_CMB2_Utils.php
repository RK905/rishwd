...
class FILETRIP_CMB2_Utils {
	 ...
	 * @since  1.0.1
	 * @return string URL to FILETRIP_CMB resources
	 */
	public function url( $path = '' ) {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$content_url = str_replace( $content_dir, WP_CONTENT_URL, filetrip_cmb_dir() );
			$filetrip_cmb_url = str_replace( DIRECTORY_SEPARATOR, '/', $content_url );

		} else {
		  $filetrip_cmb_url = str_replace(
		...
		/**
		 * Filter the FILETRIP_CMB location url
		 *
		 * @param string $filetrip_cmb_url Currently registered url
		 */
		$this->url = trailingslashit( apply_filters( 'filetrip_cmb_meta_box_url', set_url_scheme( $filetrip_cmb_url ), FILETRIP_CMB2_VERSION ) );

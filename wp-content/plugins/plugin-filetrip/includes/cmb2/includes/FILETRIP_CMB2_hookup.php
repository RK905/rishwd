...
class FILETRIP_CMB2_hookup {
	...
	public function hooks() {
		// Handle oembed Ajax
		$this->once( 'wp_ajax_filetrip_cmb_oembed_handler', array( filetrip_cmb_ajax(), 'oembed_handler' ) );
		$this->once( 'wp_ajax_nopriv_filetrip_cmb_oembed_handler', array( filetrip_cmb_ajax(), 'oembed_handler' ) );

		foreach ( get_class_methods( 'FILETRIP_CMB2_Show_Filters' ) as $filter ) {
			add_filter( 'filetrip_cmb_show_on', array( 'FILETRIP_CMB2_Show_Filters', $filter ), 10, 3 );
		}
	...
	/**
	 * Registers scripts and styles for FILETRIP_CMB
	 * @since  1.0.0
	 */
	...
	public static function register_scripts() {
		...
		if ( ! is_admin() ) {
			...
			wp_register_script( 'iris', admin_url( 'js/iris.min.js' ), array( 'jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch' ), FILETRIP_CMB2_VERSION );
			wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), FILETRIP_CMB2_VERSION );
		...
		wp_register_script( 'cmb-timepicker', filetrip_cmb_utils()->url( 'js/jquery.timePicker.min.js' ) );
		...
		wp_register_script( 'cmb2-scripts', filetrip_cmb_utils()->url( "js/cmb2{$min}.js" ), $scripts, FILETRIP_CMB2_VERSION );

		wp_localize_script( 'cmb2-scripts', 'filetrip_cmb_l10', apply_filters( 'filetrip_cmb_localized_data', array(
		...
		wp_register_style( 'cmb2-styles', filetrip_cmb_utils()->url( "css/cmb2{$min}.css" ), $styles );

		self::$registration_done = true;
	}

	/**
	 * Enqueues scripts and styles for FILETRIP_CMB
	 * @since  1.0.0
	 */
	...
	public function show_on() {
		return (bool) apply_filters( 'filetrip_cmb_show_on', true, $this->cmb->meta_box, $this->cmb );
	}
	...
	/**
	 * Includes FILETRIP_CMB styles
	 * @since  2.0.0
	 */
	public static function enqueue_cmb_css() {
		FILETRIP_CMB2_hookup::register_scripts();

		return wp_enqueue_style( 'cmb2-styles' );
	}

	/**
	 * Includes FILETRIP_CMB JS
	 * @since  2.0.0
	 */

...
/**
 * Retrieves an instance of FILETRIP_CMB2_Option based on the option key
 */
class FILETRIP_CMB2_Options {
	/**
	 * Array of all FILETRIP_CMB2_Option instances
	 * @var   array
	 * @since 1.0.0
	 */
	...
	public static function get( $option_key ) {
		...
		if ( empty( self::$option_sets ) || empty( self::$option_sets[ $option_key ] ) ) {
			self::$option_sets[ $option_key ] = new FILETRIP_CMB2_Option( $option_key );
		}
...
class FILETRIP_CMB2_Option {
	 ...
	 * @uses apply_filters() Calls 'filetrip_cmb_override_option_save_{$this->key}' hook to allow
	...
	function set( $options = false ) {
		...
		$test_save = apply_filters( "filetrip_cmb_override_option_save_{$this->key}", 'filetrip_cmb_no_override_option_save', $this->options, $this );

		if ( $test_save !== 'filetrip_cmb_no_override_option_save' ) {
			return $test_save;
		}
	 ...
	 * @uses apply_filters() Calls 'filetrip_cmb_override_option_get_{$this->key}' hook to allow
	...
	function get_options( $default = null ) {
		if ( empty( $this->options ) ) {
			...
			$test_get = apply_filters( "filetrip_cmb_override_option_get_{$this->key}", 'filetrip_cmb_no_override_option_get', $default, $this );

			if ( $test_get !== 'filetrip_cmb_no_override_option_get' ) {

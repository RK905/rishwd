...
class FILETRIP_CMB2_Types {
	...
	/**
	 * Current FILETRIP_CMB2_Field field object
	 * @var   FILETRIP_CMB2_Field object
	 * @since 1.0.0
	 */
	public $field;

	public function __construct( FILETRIP_CMB2_Field $field ) {
		$this->field = $field;
	}

	/**
	 * Default fallback. Allows rendering fields via "filetrip_cmb_render_$name" hook
	 * @since  1.0.0
	...
	public function __call( $name, $arguments ) {
		 ...
		 *
		 * @param array  $field              The passed in `FILETRIP_CMB2_Field` object
		 ...
		 * @param object $field_type_object  This `FILETRIP_CMB2_Types` object
		 */
		do_action( "filetrip_cmb_render_$name", $this->field, $this->field->escaped_value(), $this->field->object_id, $this->field->object_type, $this );
	}
	...
	public function is_valid_img_ext( $file ) {
		...
		$is_valid_types = (array) apply_filters( 'filetrip_cmb_valid_img_types', array( 'jpg', 'jpeg', 'png', 'gif', 'ico', 'icon' ) );
	...
	public function parse_args( $args, $element, $defaults ) {
		return wp_parse_args( apply_filters( "filetrip_cmb_{$element}_attributes", $this->field->maybe_set_attributes( $args ), $defaults, $this->field, $this ), $defaults );
	}
	...
	public function select_timezone() {
		$this->field->args['default'] = $this->field->args( 'default' )
			? $this->field->args( 'default' )
			: filetrip_cmb_utils()->timezone_string();
	...
	public function taxonomy_select() {
		...
		if( ! empty( $option_none ) ) {
			$option_none_value = apply_filters( 'filetrip_cmb_taxonomy_select_default_value', '' );
			$option_none_value = apply_filters( "filetrip_cmb_taxonomy_select_{$this->_id()}_default_value", $option_none_value );
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			...
			$option_none  = $this->field->args( 'show_option_none' );
			if( ! empty( $option_none ) ) {
				$option_none_value = apply_filters( "filetrip_cmb_taxonomy_radio_{$this->_id()}_default_value", apply_filters( 'filetrip_cmb_taxonomy_radio_default_value', '' ) );
				$args = array(
					'value' => $option_none_value,
					'label' => $option_none,
				);
	...
	public function file() {
		...
		$this->field = new FILETRIP_CMB2_Field( array(
			'field_args'  => $args,
		...
		if ( $meta_value && ! $_id_value ) {
			$_id_value = filetrip_cmb_utils()->image_id_from_url( esc_url_raw( $meta_value ) );
		}
	...
	public function oembed() {
		...
		'<div id="',$this->_id( '-status' ) ,'" class="cmb2-media-status ui-helper-clearfix embed_wrap">';
			...
			if ( $meta_value = $this->field->escaped_value() ) {
				echo filetrip_cmb_get_oembed( array(
					'url'         => $meta_value,
					'object_id'   => $this->field->object_id,
					'object_type' => $this->field->object_type,

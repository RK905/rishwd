 ...
 */
window.FILETRIP_CMB2 = (function(window, document, $, undefined){
	'use strict';

	// localization strings
	var l10n = window.filetrip_cmb_l10;
	var setTimeout = window.setTimeout;

	// FILETRIP_CMB functionality object
	var cmb = {
		formfield          : '',
		idNumber           : false,
		file_frames        : {},
	...
	cmb.init = function() {
		...
		cmb.log( 'FILETRIP_CMB2 localized data', l10n );
		...
		$metabox
			.on( 'change', '.filetrip_cmb_upload_file', function() {
			...
			.on( 'filetrip_cmb_remove_row', '.cmb-repeatable-group', cmb.resetTitlesAndIterator )
		...
		if ( $repeatGroup.length ) {
			$repeatGroup
				...
				.on( 'filetrip_cmb_add_row', cmb.emptyValue );
		}
	...
	cmb.addGroupRow = function( evt ) {
		...
		$self.trigger( 'filetrip_cmb_add_group_row_start', $self );
		...
		$table.trigger( 'filetrip_cmb_add_row', $newRow );
	};
	...
	cmb.addAjaxRow = function( evt ) {
		...
		$table.trigger( 'filetrip_cmb_add_row', $row );
	...
	cmb.removeGroupRow = function( evt ) {
		...
		if ( number > 1 ) {
			...
			$table.trigger( 'filetrip_cmb_remove_group_row_start', $self );
			...
			$table.trigger( 'filetrip_cmb_remove_row' );
		}
	...
	cmb.shiftRows = function( evt ) {
		...
		$self.trigger( 'filetrip_cmb_shift_rows_enter', $self );
		...
		// we're gonna shift
		$self.trigger( 'filetrip_cmb_shift_rows_start', $self );
		...
		// shift done
		$self.trigger( 'filetrip_cmb_shift_rows_complete', $self );
	};
		...
		// only proceed if the user has pasted, pressed a number, letter, or whitelisted characters
			...
			setTimeout( function() {
				...
				$.ajax({
					...
					url : l10n.ajaxurl,
					data : {
						'action': 'filetrip_cmb_oembed_handler',
						'oembed_url': oembed_url,
						'oembed_width': oembed_width > 300 ? oembed_width : 300,
						'field_id': field_id,
						'object_id': $obj.data('objectid'),
						'object_type': $obj.data('objecttype'),
						'filetrip_cmb_ajax_nonce': l10n.ajax_nonce
					},

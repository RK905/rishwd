 ...
 * extend this class
 */
abstract class FILETRIP_BKP_Service {
	...
	/**
	 * The instance FILETRIP_BKP_Backup_Schedule that this service is
	 * is currently working with
	 *
	 * @var FILETRIP_BKP_Scheduled_Backup
	 */
	protected $schedule;
	 ...
	 *
	 * @see  FILETRIP_Backup::do_action for a list of the actions
	 */
	...
	public function save() {
		...
		$new_data = isset( $_POST[$classname] ) ? sanitize_text_field($_POST[$classname]) : array();
	 ...
	 *
	 * @param FILETRIP_BKP_Scheduled_Backup $schedule An instantiated schedule object
	 */
	public function set_schedule( FILETRIP_BKP_Scheduled_Backup $schedule ) {
	...
	protected function fetch_destination_settings() {
		...
		$schedules_obj = FILETRIP_BKP_Schedules::get_instance();
 ...
 */
class FILETRIP_BKP_Services {

	/**
	 * Store the current instance
	 *
	 * @access private
	 * @var object FILETRIP_BKP_Services
	 * @static
	 */
	 ...
	 *
	 * @access private
	 * @var object FILETRIP_BKP_Scheduled_Backup
	 */
	private $schedule;
	...
	public static function instance() {
		...
		if ( ! isset( self::$instance ) )
			self::$instance = new FILETRIP_BKP_Services;
	 ...
	 *
	 * @param FILETRIP_BKP_Scheduled_Backup $schedule
	 * @return FILETRIP_BKP_SERVICE[]
	 */
	public static function get_services( FILETRIP_BKP_Scheduled_Backup $schedule = null ) {
	...
	public static function register( $filepath, $classname ) {
		...
		if ( ! file_exists( $filepath ) )
			return new WP_Error( 'filetrip_bkp_invalid_path_error', sprintf( __( 'Argument 1 for %s must be a valid filepath', 'filetrip-plugin' ), __METHOD__ ) );
	...
	public static function unregister( $filepath ) {
		...
		if ( ! isset( self::instance()->services[ $filepath ] ) )
			return new WP_Error( 'filetrip_bkp_unrecognized_service_error', sprintf( __( 'Argument 1 for %s must be a registered service', 'filetrip-plugin' ), __METHOD__ ) );
	...
	private static function instantiate( $classname ) {
		...
		if ( ! class_exists( $classname ) )
			return new WP_Error( 'filetrip_bkp_invalid_type_error', sprintf( __( 'Argument 1 for %s must be a valid class', 'filetrip-plugin' ) ), __METHOD__ );

		/**
		 * @var FILETRIP_BKP_Service
		 */

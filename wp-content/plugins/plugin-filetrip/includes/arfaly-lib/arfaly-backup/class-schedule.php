 ...
 *
 * @extends FILETRIP_Backup
 */
class FILETRIP_BKP_Scheduled_Backup extends FILETRIP_Backup {
	...
	public function __construct( $id ) {
		...
		// Load the options
		$this->options = array_filter( (array) get_option( 'filetrip_bkp_schedule_' . $this->get_id() ) );

		// Some properties can be overridden with defines
		if ( defined( 'FILETRIP_BKP_ROOT' ) && FILETRIP_BKP_ROOT ) {
			$this->set_root( FILETRIP_BKP_ROOT );
		}

		if ( defined( 'FILETRIP_BKP_PATH' ) && FILETRIP_BKP_PATH ) {
			$this->set_path( FILETRIP_BKP_PATH );
		}

		if ( defined( 'FILETRIP_BKP_EXCLUDE' ) && FILETRIP_BKP_EXCLUDE ) {
			parent::set_excludes( FILETRIP_BKP_EXCLUDE, true );
		}

		parent::set_excludes( $this->default_excludes(), true );

		if ( defined( 'FILETRIP_BKP_MYSQLDUMP_PATH' ) ) {
			$this->set_mysqldump_command_path( FILETRIP_BKP_MYSQLDUMP_PATH );
		}

		if ( defined( 'FILETRIP_BKP_ZIP_PATH' ) ) {
			$this->set_zip_command_path( FILETRIP_BKP_ZIP_PATH );
		}

		if ( defined( 'FILETRIP_BKP_ZIP_PATH' ) && FILETRIP_BKP_ZIP_PATH === 'PclZip' && $this->skip_zip_archive = true ) {
			$this->set_zip_command_path( false );
		}

		if ( defined( 'FILETRIP_BKP_SCHEDULE_START_TIME' ) && strtotime( 'FILETRIP_BKP_SCHEDULE_START_TIME' ) ) {
			$this->set_schedule_start_time( strtotime( 'FILETRIP_BKP_SCHEDULE_START_TIME' ) );
		}
	...
	public function set_excludes( $excludes, $append = false ) {
		...
		// Use the validation from FILETRIP_Backup::set_excludes
	...
	public function set_max_backups( $max ) {
		...
		if ( empty( $max ) || ! is_int( $max ) ) {
			return new WP_Error( 'filetrip_bkp_invalid_type_error', sprintf( __( 'Argument 1 for %s must be a valid integer', 'filetrip-plugin' ), __METHOD__ ) );
		}
	...
	function is_site_size_being_calculated() {
		return get_transient( 'filetrip_bkp_directory_filesizes_running' );
	}
	...
	function is_site_size_cached() {
		return get_transient( 'filetrip_bkp_directory_filesizes' );
	}
	...
	public function list_directory_by_total_filesize( $directory ) {
		...
		while ( $file_handle = readdir( $handle ) ) {
			...
			$file = new SplFileInfo( FILETRIP_Backup::conform_dir( trailingslashit( $directory ) . $file_handle ) );
	 ...
	 *
	 * Locks should be set by the caller with `set_transient( 'filetrip_bkp_directory_filesizes_running', true, HOUR_IN_SECONDS );`
	 *
	...
	public function recursive_filesize_scanner() {
		...
		$directory_sizes = get_transient( 'filetrip_bkp_directory_filesizes' );

		// If we do have it in cache then let's use it and also clear the lock
		if ( is_array( $directory_sizes ) ) {

			delete_transient( 'filetrip_bkp_directory_filesizes_running' );
		...
		set_transient( 'filetrip_bkp_directory_filesizes', $directory_sizes, DAY_IN_SECONDS );
		...
		if ( $file->isDir() ) {
			...
			if ( ! is_array( $directory_sizes ) ) {
				...
				if ( ! $this->is_site_size_being_calculated() ) {
					...
					set_transient( 'filetrip_bkp_directory_filesizes_running', true, HOUR_IN_SECONDS );

					// Schedule a Backdrop task to trigger a recalculation
					$task = new FILETRIP_Backdrop_Task( array( $this, 'recursive_filesize_scanner' ) );
					$task->schedule();
			...
			if ( $skip_excluded_files ) {
				...
				foreach ( $directory_sizes as $path => $size ) {
					...
					if ( $excludes && preg_match( '(' . $excludes . ')', str_ireplace( $root, '', FILETRIP_Backup::conform_dir( $path ) ) ) ) {
						unset( $directory_sizes[ $path ] );
					}
	...
	public function set_schedule_start_time( $time ) {
		...
		if ( (int) $time <= time() ) {
			return new WP_Error( 'filetrip_bkp_invalid_argument_error', sprintf( __( 'Argument 1 for %s must be a valid future timestamp', 'filetrip-plugin' ), __METHOD__ ) );
		}
	...
	public function set_reoccurrence( $reoccurrence ) {
		...
		$filetrip_bkp_schedules = $this->get_cron_schedules();

		// Check it's valid
		if ( ! is_string( $reoccurrence ) || ! trim( $reoccurrence ) || ( ! in_array( $reoccurrence, array_keys( $filetrip_bkp_schedules ) ) ) && 'manually' !== $reoccurrence ) {
			return new WP_Error( 'filetrip_bkp_invalid_argument_error', sprintf( __( 'Argument 1 for %s must be a valid cron reoccurrence or "manually"', 'filetrip-plugin' ), __METHOD__ ) );
		}
	...
	public function get_interval() {
		...
		return $filetrip_bkp_schedules[ $this->get_reoccurrence() ]['interval'];
	...
	public function get_next_occurrence( $gmt = true ) {
		...
		$time = wp_next_scheduled( 'filetrip_bkp_schedule_hook', array( $this->get_id() ) );
	...
	public function schedule() {
		...
		wp_schedule_event( $schedule_timestamp, $this->get_reoccurrence(), 'filetrip_bkp_schedule_hook', array( $this->get_id() ) );
	...
	public function unschedule() {
		wp_clear_scheduled_hook( 'filetrip_bkp_schedule_hook', array( $this->get_id() ) );
	}
	...
	public function run() {
		...
		$this->set_status( __( 'Starting Backup', 'filetrip-plugin' ) );
	...
	protected function do_action( $action ) {
		...
		foreach ( FILETRIP_BKP_Services::get_services( $this ) as $service ) {
		...
		switch ( $action ) :
			...
			case 'filetrip_bkp_mysqldump_started' :

				$this->set_status( sprintf( __( 'Dumping Database %s', 'filetrip-plugin' ), '(<code>' . $this->get_mysqldump_method() . '</code>)' ) );
				break;

			case 'filetrip_bkp_mysqldump_verify_started' :

				$this->set_status( sprintf( __( 'Verifying Database Dump %s', 'filetrip-plugin' ), '(<code>' . $this->get_mysqldump_method() . '</code>)' ) );
				break;

			case 'filetrip_bkp_archive_started' :

				$this->set_status( sprintf( __( 'Creating zip archive %s', 'filetrip-plugin' ), '(<code>' . $this->get_archive_method() . '</code>)' ) );
				break;

			case 'filetrip_bkp_archive_verify_started' :

				$this->set_status( sprintf( __( 'Verifying Zip Archive %s', 'filetrip-plugin' ), '(<code>' . $this->get_archive_method() . '</code>)' ) );
				break;

			case 'filetrip_bkp_backup_complete' :

				$this->set_status( __( 'Finishing Backup', 'filetrip-plugin' ) );
				break;

			case 'filetrip_bkp_error' :
		...
		do_action( 'filetrip_bkp_action_complete', $action, $this );
	}
	...
	public function delete_backup( $filepath ) {
		...
		if ( empty( $filepath ) || ! is_string( $filepath ) ) {
			return new WP_Error( 'filetrip_bkp_empty_string_error', sprintf( __( 'Argument 1 for %s must be a non empty string', 'filetrip-plugin' ), __METHOD__ ) );
		}

		// Make sure it exists
		if ( ! file_exists( $filepath ) ) {
			return new WP_Error( 'filetrip_bkp_file_error', sprintf( __( '%s doesn\'t exist', 'filetrip-plugin' ), $filepath ) );
		}

		// Make sure it was created by this schedule
		if ( strpos( $filepath, $this->get_id() ) === false ) {
			return new WP_Error( 'filetrip_bkp_backup_error', __( 'That backup wasn\'t created by this schedule', 'filetrip-plugin' ) );
		}
	...
	public function save() {
		...
		if ( $this->options !== get_option( 'filetrip_bkp_schedule_' . $this->get_id() ) ) {
			update_option( 'filetrip_bkp_schedule_' . $this->get_id(), $this->options );
		}
	...
	public function cancel() {
		...
		delete_option( 'filetrip_bkp_schedule_' . $this->get_id() );
	...
	public function default_excludes() {
		...
		$filetrip_bkp_folders = array_merge(
			$this->find_backup_folders( 'filetrip-', WP_CONTENT_DIR ),
			$this->find_backup_folders( 'filetrip-', $hmn_upload_dir['path'] )
		);

		if ( ! empty( $filetrip_bkp_folders ) ) {
			foreach ( $filetrip_bkp_folders as $path ) {
				$excluded[] = $path;
			}
		}
		...
		return apply_filters( 'filetrip_bkp_default_excludes', $excluded );
	}
	...
	protected function find_backup_folders( $needle, $haystack ) {
		...
		if ( ! empty( $folders_to_search ) ) {
			...
			foreach ( $folders_to_search as $folder ) {
				...
				$default_path = get_option( 'filetrip_bkp_default_path' );

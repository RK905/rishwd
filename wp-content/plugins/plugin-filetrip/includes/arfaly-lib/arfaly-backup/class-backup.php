...
class Filetrip_Backup {
	...
	public static function is_shell_exec_available() {
		...
		if ( ! @shell_exec( 'echo arfaly_uploader' ) )
			return false;
	...
	public function set_archive_filename( $filename ) {
		...
		if ( empty( $filename ) || ! is_string( $filename ) )
			return new WP_Error( 'invalid_file_name', __( 'archive filename must be a non empty string', 'filetrip-plugin' ) );

		if ( pathinfo( $filename, PATHINFO_EXTENSION ) !== 'zip' )
			return new WP_Error( 'invalid_file_extension', sprintf( __( 'invalid file extension for archive filename <code>%s</code>', 'filetrip-plugin' ), $filename ) );
	...
	public function set_database_dump_filename( $filename ) {
		...
		if ( empty( $filename ) || ! is_string( $filename ) )
			return new WP_Error( 'invalid_file_name', __( 'database dump filename must be a non empty string', 'filetrip-plugin' ) );

		if ( pathinfo( $filename, PATHINFO_EXTENSION ) !== 'sql' )
			return new WP_Error( 'invalid_file_extension', sprintf( __( 'invalid file extension for database dump filename <code>%s</code>', 'filetrip-plugin' ), $filename ) );
	...
	public function set_root( $path ) {
		...
		if ( empty( $path ) || ! is_string( $path ) || ! is_dir( $path ) )
			return new WP_Error( 'invalid_directory_path', sprintf( __( 'Invalid root path <code>%s</code> must be a valid directory path', 'filetrip-plugin' ), $path ) );
	...
	public function get_path() {
		...
		if ( empty( $this->path ) )
			$this->set_path( self::conform_dir( filetrip_bkp_path() ) );
	...
	public function set_path( $path ) {
		...
		if ( empty( $path ) || ! is_string( $path ) )
			return new WP_Error( 'invalid_backup_path', sprintf( __( 'Invalid backup path <code>%s</code> must be a non empty (string)', 'filetrip-plugin' ), $path ) );
	...
	public function set_type( $type ) {
		...
		if ( ! is_string( $type ) || ! in_array( $type, array( 'file', 'database', 'complete' ) ) )
			return new WP_Error( 'invalid_backup_type', sprintf( __( 'Invalid backup type <code>%s</code> must be one of (string) file, database or complete', 'filetrip-plugin' ), $type ) );
	...
	public function backup() {
		...
		$this->do_action( 'filetrip_bkp_backup_started' );
		...
		$this->do_action( 'filetrip_bkp_backup_complete' );
	...
	public function dump_database() {
		...
		$this->do_action( 'filetrip_bkp_mysqldump_finished' );
	...
	public function mysqldump() {
		...
		$this->do_action( 'filetrip_bkp_mysqldump_started' );
		...
		if ( ! defined( 'FILETRIP_BKP_MYSQLDUMP_SINGLE_TRANSACTION' ) || FILETRIP_BKP_MYSQLDUMP_SINGLE_TRANSACTION !== false )
...
//			$fetch = $f[0];
//			$curr_table =  mysqli_query($this->db,"SHOW TABLES LIKE '".$tables."'");
	...
	public function archive() {
		...
		$this->do_action( 'filetrip_bkp_archive_finished' );
	...
	public function zip() {
		...
		$this->do_action( 'filetrip_bkp_archive_started' );
	...
	public function pcl_zip() {
		...
		global $_filetrip_bkp_exclude_string;

		$_filetrip_bkp_exclude_string = $this->exclude_string( 'regex' );
		...
		// Zip up everything
		if ( $this->get_type() !== 'database' )
			if ( ! $archive->add( $this->get_root(), PCLZIP_OPT_REMOVE_PATH, $this->get_root(), PCLZIP_CB_PRE_ADD, 'filetrip_bkp_pclzip_callback' ) )
		...
		unset( $GLOBALS['_filetrip_bkp_exclude_string'] );
	...
	public function verify_mysqldump() {
		...
		$this->do_action( 'filetrip_bkp_mysqldump_verify_started' );
	...
	public function verify_archive() {
		...
		$this->do_action( 'filetrip_bkp_archive_verify_started' );
	...
	public function error( $context, $error ) {
		...
		$this->do_action( 'filetrip_bkp_error' );
	...
	private function warning( $context, $warning ) {
		...
		$this->do_action( 'filetrip_bkp_warning' );
 ...
 * @return bool
 */
function filetrip_bkp_pclzip_callback( $event, $file ) {
	...
	elseif ( $_filetrip_bkp_exclude_string && preg_match( '(' . $_filetrip_bkp_exclude_string . ')', $file['stored_filename'] ) )
		return false;
